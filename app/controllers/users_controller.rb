class UsersController < ApplicationController
  ...
  def update
    ...
    if !@user.locked
      ...
      msg = 'Email & account updated successfully. Please check your mail for confirmation instructions.' if update_user_params[:email].present?

      # Make sure that current_password and password are present if the user is updating their password
      if update_user_params[:password].present? && update_user_params[:current_password].present? && !@user.authenticate(update_user_params[:current_password])
      ...
      if @user.update(update_user_params)
        @user.send_confirmation_email! if update_user_params[:email].present?
