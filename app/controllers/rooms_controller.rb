class RoomsController < ApplicationController
  ...
  def create
    ...
    if @room.save
      ...
      ResourceInviteService.new(params[:invited_users], current_user.id, @room).create if params[:invited_users].present?
  ...
  def update
    ...
    # params[:invited_users] is already a JSON so we pass it as it is to the next
    # steps as Sidekiq is expecting this format.
    ResourceInviteService.new(params[:invited_users], current_user.id, @room).update if params[:invited_users].present?
  ...
  def room_params
    params.require(:room).permit(:allow_anonymous, :description, :public_description, :room_type, :name, :start_date, :status, :end_date)
  end
end
